...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#edeff1"}'>
    <el-form
      ...
      label-width="150px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="退还编号" prop="tuihaibianhao">
              <el-input v-model="ruleForm.tuihaibianhao" placeholder="退还编号" disabled></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="扣除押金" prop="kouchuyajin">
			<el-input-number v-model="ruleForm.kouchuyajin" placeholder="扣除押金" :disabled=" false ||ro.kouchuyajin"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="退还押金" prop="tuihaiyajin">
              <el-input v-model="tuihaiyajin" placeholder="退还押金" disabled></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="退还日期" prop="tuihairiqi">
              <el-date-picker
				  :disabled=" false  ||ro.tuihairiqi"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`yajintuihai/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
		this.ruleForm.ispay = '未支付'
			...
			if(this.ruleForm.kouchuyajin>this.ruleForm.yajin){
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('yajintuihai/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`yajintuihai/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
