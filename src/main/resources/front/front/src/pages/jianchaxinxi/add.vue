...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#edeff1"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="检查结果" prop="jianchajieguo">
            <el-select v-model="ruleForm.jianchajieguo" placeholder="请选择检查结果" :disabled=" false  ||ro.jianchajieguo" >
              <el-option
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="检查日期" prop="jianchariqi">
              <el-date-picker
				  :disabled=" false  ||ro.jianchariqi"
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="检验员账号" prop="jianyanyuanzhanghao">
            <el-input v-model="ruleForm.jianyanyuanzhanghao" 
                placeholder="检验员账号" clearable :disabled=" false  ||ro.jianyanyuanzhanghao"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="检验员姓名" prop="jianyanyuanxingming">
            <el-input v-model="ruleForm.jianyanyuanxingming" 
                placeholder="检验员姓名" clearable :disabled=" false  ||ro.jianyanyuanxingming"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="检查详情" prop="jianchaxiangqing">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if((json.jianyanyuanzhanghao!=''&&json.jianyanyuanzhanghao) || json.jianyanyuanzhanghao==0){
                this.ruleForm.jianyanyuanzhanghao = json.jianyanyuanzhanghao;
				this.ro.jianyanyuanzhanghao = true;
            }
            if((json.jianyanyuanxingming!=''&&json.jianyanyuanxingming) || json.jianyanyuanxingming==0){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jianchaxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jianchaxinxi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`jianchaxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
