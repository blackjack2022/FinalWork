  ...
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          jianyanyuanzhanghao: [
            { required: true, message: '检验员账号不能为空', trigger: 'blur' },
          ],
          mima: [
            { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          jianyanyuanxingming: [
            { required: true, message: '检验员姓名不能为空', trigger: 'blur' },
          ],
          xingbie: [
          ],
          shouji: [
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jianyanyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jianyanyuan/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`jianyanyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
