		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="检查状态" prop="jianchazhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">检查状态</label>
						<el-select clearable v-model="searchForm.jianchazhuangtai" placeholder="请选择检查状态" >
							<el-option v-for="(item,index) in jianchazhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
			...
			<div :style='{"padding":"0","borderColor":"#eee","borderRadius":"0px","background":"none","borderWidth":"1px 0 0 1px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="jianchazhuangtai"
						label="检查状态">
						<template slot-scope="scope">
							{{scope.row.jianchazhuangtai}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="guihairiqi"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('haichejilu','检查信息')" @click="jianchaxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','jianchazhuangtai','已检查','已检查,未检查'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								检查信息
							</el-button>
							<el-button class="edit" v-if=" isAuth('haichejilu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
...
import jianchaxinxiCrossAddOrUpdate from "../jianchaxinxi/add-or-update";
	export default {
		...
		methods: {
			...
			jianchaxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.jianchaxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','haichejilu');
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
          ...
          if(this.searchForm.jianchazhuangtai!='' && this.searchForm.jianchazhuangtai!=undefined){
            params['jianchazhuangtai'] = this.searchForm.jianchazhuangtai
          }
