		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10px","fontSize":"14px","color":"inherit","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="del" v-if="isAuth('qichefenlei','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="edit" v-if=" isAuth('qichefenlei','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('qichefenlei','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
	...
	qichefenleistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'qichefenlei/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
