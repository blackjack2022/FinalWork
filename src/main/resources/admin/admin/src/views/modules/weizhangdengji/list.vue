		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10px","fontSize":"14px","color":"inherit","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('weizhangdengji','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('weizhangdengji','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
						移除
					</el-button>


					<el-button class="btn18" v-if="isAuth('weizhangdengji','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
			...
			<div :style='{"padding":"0","borderColor":"#eee","borderRadius":"0px","background":"none","borderWidth":"1px 0 0 1px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="weiguizhaopian" width="200" label="违规照片">
						<template slot-scope="scope">
							<div v-if="scope.row.weiguizhaopian">
								<img v-if="scope.row.weiguizhaopian.substring(0,4)=='http'" :src="scope.row.weiguizhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.weiguizhaopian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.weiguizhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.weiguizhaopian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('weizhangdengji','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('weizhangdengji','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								浏览
							</el-button>
							<el-button class="edit" v-if=" isAuth('weizhangdengji','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('weizhangdengji','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
	export default {
		...
		methods: {
			...
			payHandler(row){
				this.$storage.set('paytable','weizhangdengji');
	...
	weizhangdengjistatusChange(e,row){
		if(row.status==0){
